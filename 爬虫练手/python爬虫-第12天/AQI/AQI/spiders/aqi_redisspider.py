...
# 1. 导入需要使用的类： RedisSpider
from scrapy_redis.spiders import RedisSpider

from ..items import AqiItem

# 2. 修改继承的父类
#Qclass AqiSpider(scrapy.Spider):
class AqiRedisSpider(RedisSpider):
    ...
    def parse(self, response):
        ...
        for city_link in city_link_list[10:12]:
            # 发送每个城市链接的请求，并传递响应到 parse_month
            link = self.base_url + city_link
            yield scrapy.Request(link, callback=self.parse_month)
    ...
    def parse_month(self, response):
        ...
        for month_link in month_link_list[10:15]:
            # 发送每个月的链接请求，并传递响应到parse_day
            link = self.base_url + month_link
            yield scrapy.Request(link, callback=self.parse_day)
